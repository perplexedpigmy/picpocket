...
class QrCodeRoundTripTest {
    ...
    fun `generate returns bitmap for valid text`() {
        val bitmap = QrCodeGenerator.generate("docscanner-pair:device-123:MyPhone:1000")
    ...
    fun `generate handles special characters`() {
        val data = "docscanner-pair:device-abc!@#:My Phone:2000"
    ...
    fun `generate handles long URIs`() {
        ...
        val data = "docscanner-pair:$longId:Device-With-Long-ID:${Long.MAX_VALUE}"
        val bitmap = QrCodeGenerator.generate(data)
        assertNotNull(bitmap)
    }
